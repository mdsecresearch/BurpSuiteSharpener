                            ...
                            new java.util.TimerTask() {
                                ...
                                public void run() {
                                    SwingUtilities.invokeLater(() -> {
                                        ...
                                        BurpUITools.MainTabs toolName1 = BurpUITools.getMainTabsObjFromString(sharedParameters.get_rootTabbedPane().getTitleAt(sharedParameters.get_rootTabbedPane().indexOfComponent(((Component) e.getSource()).getParent())));
                                        JTabbedPane subTabbedPane1 = sharedParameters.get_toolTabbedPane(toolName1);
                                        if (subTabbedPane1 != null) {
                                            if (subTabbedPane1.getPropertyChangeListeners("tabPropertyChangeListener").length == 0)
                                                subTabbedPane1.addPropertyChangeListener("indexForTabComponent", tabPropertyChangeListener);

                                            addSubTabsListener(subTabbedPane1, toolName1);

                                            // as there is no other getComponentListeners by default, we can remove them all
                                            for (ComponentListener cl : subTabbedPane1.getComponentListeners()) {
                                                subTabbedPane1.removeComponentListener(cl);
                                            }

                                            sharedParameters.allSettings.subTabsSettings.loadSettings(toolName1);
                                            sharedParameters.allSettings.subTabsSettings.saveSettings(toolName1);
                                            set_isUpdateInProgress(false);
                                        }
                                    });
                                }
    ...
    private void addSubTabsListener(JTabbedPane subTabbedPane, BurpUITools.MainTabs toolName) {
        ...
        if (_isShortcutEnabled) {
            ...
            subTabbedPane.getActionMap().put("PreviouslySelectedTab", new AbstractAction() {
                @Override
                public void actionPerformed(ActionEvent e) {
                    SubTabsActions.jumpToPreviosulySelectedTab(sharedParameters, e);
                }
            });
